...
Market: the United States. The reader is an American clinician, health-system staffer or
family caregiver dealing with Medicare, CMS rules and US hospitals. Use the US evidence in
the list: CMS, Medicare, Federal Register, US studies and US trade reporting. Do not build
a post on research from another country, and never name a foreign country, health system
or cohort. If the only fact you have for an angle is foreign, use a different fact.
...
- Then three or four short lines that go a level deeper than the hook: what is actually
  true, the specific that proves it (a code, a date, a rule, a number from the facts
  below), and why it changes what you'd do on Monday. Line breaks between them, not a
  paragraph.
- Use the room. A post of {min_chars} to {max_chars} characters has something in it; a
...
- No em dashes or en dashes, ever. Use a comma, a colon, or start a new sentence. Nothing
  else marks writing as machine-made as fast.
